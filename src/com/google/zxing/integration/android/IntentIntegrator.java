 ...
 * </p>
 * <p>
 * {@code public void onActivityResult(int requestCode, int resultCode, Intent intent) IntentResult scanResult = IntentIntegrator.parseActivityResult(requestCode, resultCode, intent); if (scanResult
 * != null) // handle scan result } // else continue with any other code you need in the method ... } }
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * You can use {@link #initiateScan(Activity, CharSequence, CharSequence, CharSequence, CharSequence)} or {@link #initiateScan(Activity, int, int, int, int)} to customize the download prompt with
 * different text labels.
 * </p>
 * <p>
 * Note that {@link #initiateScan(Activity)} returns an {@link AlertDialog} which is non-null if the user was prompted to download the application. This lets the calling app potentially manage the
 * dialog. In particular, ideally, the app dismisses the dialog if it's still active in its {@link Activity#onPause()} method.
 * </p>
...
public final class IntentIntegrator {
    ...
    private static final String PACKAGE = "com.google.zxing.client.android"; //$NON-NLS-1$

    // supported barcode formats
    public static final String PRODUCT_CODE_TYPES = "UPC_A,UPC_E,EAN_8,EAN_13"; //$NON-NLS-1$
    public static final String ONE_D_CODE_TYPES = PRODUCT_CODE_TYPES + ",CODE_39,CODE_93,CODE_128"; //$NON-NLS-1$
    public static final String QR_CODE_TYPES = "QR_CODE"; //$NON-NLS-1$
    ...
    /**
     * See {@link #initiateScan(Activity, CharSequence, CharSequence, CharSequence, CharSequence)} -- same, but uses default English labels.
     */
    public static AlertDialog initiateScan(Activity activity) {
        return initiateScan(activity, activity.getString(R.string.bsi_default_title), activity.getString(R.string.bsi_default_message), activity.getString(R.string.bsi_yes), activity.getString(R.string.bsi_no));
    }

    /**
     * See {@link #initiateScan(Activity, CharSequence, CharSequence, CharSequence, CharSequence, CharSequence)} -- same, but scans for all supported barcode types.
     * 
     ...
     * @param stringButtonYes text of button user clicks when agreeing to download Barcode Scanner (e.g. "Yes")
     * @param stringButtonNo text of button user clicks when declining to download Barcode Scanner (e.g. "No")
     * @param stringDesiredBarcodeFormats a comma separated list of codes you would like to scan for.
     * @return an {@link AlertDialog} if the user was prompted to download the app, null otherwise
    ...
    public static AlertDialog initiateScan(Activity activity, CharSequence stringTitle, CharSequence stringMessage, CharSequence stringButtonYes, CharSequence stringButtonNo, CharSequence stringDesiredBarcodeFormats) {
        Intent intentScan = new Intent(PACKAGE + ".SCAN"); //$NON-NLS-1$
        ...
        if (stringDesiredBarcodeFormats != null) {
            ...
            intentScan.putExtra("SCAN_FORMATS", stringDesiredBarcodeFormats); //$NON-NLS-1$
        }
    ...
    private static AlertDialog showDownloadDialog(final Activity activity, CharSequence stringTitle, CharSequence stringMessage, CharSequence stringButtonYes, CharSequence stringButtonNo) {
        ...
        downloadDialog.setPositiveButton(stringButtonYes, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialogInterface, int i) {
                Uri uri = Uri.parse("market://search?q=pname:" + PACKAGE); //$NON-NLS-1$
     ...
     * </p>
     * 
     * @return null if the event handled here was not related to {@link IntentIntegrator}, or else an {@link IntentResult} containing the result of the scan. If the user cancelled scanning, the fields
     *         will be null.
     */
    public static IntentResult parseActivityResult(int requestCode, int resultCode, Intent intent) {
        if (requestCode == REQUEST_CODE) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                String formatName = intent.getStringExtra("SCAN_RESULT_FORMAT"); //$NON-NLS-1$
    ...
    /**
     * See {@link #shareText(Activity, CharSequence, CharSequence, CharSequence, CharSequence, CharSequence)} -- same, but uses default English labels.
     */
    public static void shareText(Activity activity, CharSequence text) {
        shareText(activity, text, activity.getString(R.string.bsi_default_title), activity.getString(R.string.bsi_default_message), activity.getString(R.string.bsi_yes), activity.getString(R.string.bsi_no));
    }

    /**
     * See {@link #shareText(Activity, CharSequence, CharSequence, CharSequence, CharSequence, CharSequence)} -- same, but takes string IDs which refer to the {@link Activity}'s resource bundle
     * entries.
     */
    ...
    public static void shareText(Activity activity, CharSequence text, CharSequence stringTitle, CharSequence stringMessage, CharSequence stringButtonYes, CharSequence stringButtonNo) {
        ...
        intent.setAction(PACKAGE + ".ENCODE"); //$NON-NLS-1$
        intent.setPackage(PACKAGE);
        intent.putExtra("ENCODE_TYPE", "TEXT_TYPE"); //$NON-NLS-1$ //$NON-NLS-2$
        intent.putExtra("ENCODE_DATA", text); //$NON-NLS-1$
        try {
            activity.startActivity(intent);
        } catch (ActivityNotFoundException e) {
